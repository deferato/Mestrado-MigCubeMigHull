...
#define SUPERSTEPS 80 // mexer aqui!!!
#define PROCESSES 60
#define ALPHA 8
...
int bsp(int argc, char *argv[])
{
	unsigned int i,j, k, instructions;
	msg_task_t compute = NULL;
	char mailbox_send[PROCESSES-1][256], mailbox_recv[PROCESSES-1][256];
	msg_task_t task_send[PROCESSES-1], task_recv[PROCESSES-1];
	msg_comm_t async_send[PROCESSES-1], async_recv[PROCESSES-1];
	...
	instructions = LINES * LINES_PER_PROCESSES * 100;

	time_communication_start = (double *) init_vector_communication(1);	
	amount_communication = (int *)init_vector_communication(0);	

	for (i=0; i< SUPERSTEPS2 ; i++)
	{
		if (MSG_process_self_PID() == 1)
		{
			XBT_INFO("Superstep %d", i+1);
		...
		if (i %2 ==0)
		{
			compute = MSG_task_create("computing", LINES * LINES_PER_PROCESSES * 100, 0, NULL);
			time_computation_start = MSG_get_clock();		
			MSG_task_execute(compute);
			MSG_task_destroy(compute);
			time_computation_end = MSG_get_clock();
			for (j=1, k=0; j <= PROCESSES; j++)
			{
				if (MSG_process_self_PID() != j)
				{
					sprintf(mailbox_send[k], "process-%d-%d", MSG_process_self_PID(), j);
					
					sprintf(mailbox_recv[k], "process-%d-%d", j, MSG_process_self_PID());
		
					index = verify_set(mailbox_recv[k], -1);
					time_communication_start[index] = MSG_get_clock();

					task_send[k] = MSG_task_create("sending", 0, (((PROCESSES-1)*LINES_PER_PROCESSES) * ELEMENTS_IN_A_LINE), NULL);
					async_send[k] = MSG_task_isend(task_send[k], mailbox_send[k]);
					task_recv[k] = NULL;
					amount_communication[index] = amount_communication[index] + (((PROCESSES-1)*LINES_PER_PROCESSES) * ELEMENTS_IN_A_LINE);
					async_recv[k] = MSG_task_irecv(&(task_recv[k]), mailbox_recv[k]);
					k++;
					time_communication_start[index] = MSG_get_clock() - time_communication_start[index];
				}
			}
			...
			for (j=1, k=0; j <= PROCESSES; j++)
			{
				if (MSG_process_self_PID() != j)
				{
					MSG_comm_wait(async_recv[k], -1);
					MSG_comm_wait(async_send[k], -1);
					k++;
				}
			}
		}
		...
		{
			compute = MSG_task_create("computing", COLUMNS * ELEMENTS_IN_A_LINE * 100, 0, NULL);
			time_computation_start = MSG_get_clock();		
			MSG_task_execute(compute);
			MSG_task_destroy(compute);
			time_computation_end = MSG_get_clock();
